...
export default function CreateAppointments({navigation}){
...
const createSlots=()=>{
  ...
  function calculate_time_slot(start_time, end_time, interval = "30"){
    ...
    var time_slots = new Array();
      ...
      time_slots.push({startTime:formatted_time,date:date,slotTime:slotTime,did:user._id}); //need to change did
    }
    return time_slots;
  }
 ...
 const saveSlots=()=>{
    ...
    var requestOptions= {
      ...
      fetch(API_URL+'/appointment/new',requestOptions).then((response)=>response.json()).then((result)=>{
          console.log(result);
          Alert.alert(
  ...
  return (<SafeAreaView>
       ...
       <View style={{ backgroundColor:'#f2333f', padding:'5%',paddingTop:'10%',marginBottom:'5%', justifyContent:'space-between'}}>
                ...
                <View style={{alignItems:'center'}} >
                    <Text style={{alignSelf:'center', color:'white', fontSize:18, fontWeight:'bold'}}>Create Appointment Slots</Text>
                </View>
      ...
      >
        <View style={{alignItems:'center', justifyContent:'center',flex:1,flexDirection:'column', backgroundColor:'white'}}>
          <Text style={{fontSize:35,color:'white'}}>Select Slot Size</Text>
        ...
        <View style={{ alignItems:'center'}}>
          ...
          <TouchableOpacity style={{backgroundColor:active, alignItems:'center', padding:'10%', borderRadius:20}} onPress={()=>showMode('date')}>
            <View><Text style={{fontSize:22,color:'white'}}>{date.toLocaleDateString()}</Text></View>
          </TouchableOpacity>
          </View>
        </View>
...
<View style={{flexDirection:'row', justifyContent:'space-around'}}>
        <View style={{margin:'5%', alignItems:'center'}}>
          ...
          <TouchableOpacity style={{backgroundColor:active, alignItems:'center', padding:'10%', borderRadius:20, }} onPress={()=>showStartTime()}>
            <View><Text style={{fontSize:22,color:'white'}}>{startTime.toLocaleTimeString().substring(0,5)}</Text></View>
          </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={{margin:'5%', alignItems:'center'}}>
          ...
          <TouchableOpacity style={{backgroundColor:active, alignItems:'center', padding:'10%', borderRadius:20}} onPress={()=>showEndTime()}>
            <View><Text style={{fontSize:22,color:'white'}}>{endTime.toLocaleTimeString().substring(0,5)}</Text></View>
          </TouchableOpacity>
          </View>
        </View>

        <View style={{margin:'5%', alignItems:'center'}}>
          <Text style={{fontSize:18}}>Slot Time</Text>
            <View style={{ width:'100%',}}>
            <TouchableOpacity style={{backgroundColor:active, alignItems:'center', padding:'10%', borderRadius:20}} onPress={() => setModalVisible(!modalVisible)}>
              <View><Text style={{fontSize:22,color:'white'}}>{slotTime}m</Text></View>
            </TouchableOpacity>
          </View>
...
<View style={{flexDirection:'row',justifyContent:'space-around',paddingBottom:'2%'}}>
        <View>
        <Button  title='create slots' color={active} onPress={createSlots} />
        </View>
        <View>
        <Button title='Save Slots' color={'blue'} onPress={saveSlots}/>
        </View>
      </View>
</View>
